...
def setup_cluster(conn, main_nodes, subordinate_nodes, opts, deploy_ssh_key):
    main = get_dns_name(opts, main_nodes[0])
    if deploy_ssh_key:
        ...
        for subordinate in subordinate_nodes:
            print get_ip_address(opts, subordinate)
            ssh_write(get_ip_address(opts, subordinate), opts, ['tar', 'x'], dot_ssh_tar)
...
def deploy_files(conn, root_dir, opts, main_nodes, subordinate_nodes, modules):
    active_main = get_dns_name(opts, main_nodes[0])
    ...
    template_vars = {
        "main_list": '\n'.join([get_dns_name(opts, i) for i in main_nodes]),
        "active_main": active_main,
        "subordinate_list": '\n'.join([get_dns_name(opts, i) for i in subordinate_nodes]),
...
def real_main():
    ...
    if action == "launch":
        ...
        (main_nodes, subordinate_nodes) = get_existing_cluster(conn, opts, cluster_name)
        print get_dns_name(opts, main_nodes[0])
